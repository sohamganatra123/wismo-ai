...
const base: GmailPayload = {
  ...
  payload: {
    headers: [
      { name: "From", value: "Amina <amina@example.com>" },
      { name: "To", value: "help@example.com" },
      { name: "Subject", value: "Where is order 4921?" },
...
describe("Gmail normalization", () => {
  it("keeps IDs, headers, timestamp, and plain text", () => {
    const result = normalizeGmailMessage({
      ...
      payload: {
        ...base.payload,
        mimeType: "text/plain",
        body: { data: encoded("Original email text") },
      },
    });
    expect(result).toMatchObject({
      ...
      messageIdHeader: "<mail-1@example.com>",
      sentAt: 1000,
    });
  });
    ...
    const result = normalizeGmailMessage({
      ...
      payload: {
        ...
        parts: [
          {
            mimeType: "text/html",
            body: { data: encoded("<p>HTML copy</p>") },
          },
          { mimeType: "text/plain", body: { data: encoded("Plain copy") } },
        ],
      },
    });
  ...
  it("uses readable HTML when plain text is absent", () => {
    const result = normalizeGmailMessage({
      ...
      payload: {
        ...base.payload,
        mimeType: "text/html",
        body: { data: encoded("<p>Hello<br>there &amp; thanks</p>") },
      },
    });
  ...
  it("uses safe header defaults", () => {
    expect(
      normalizeGmailMessage({ id: "m", threadId: "t", payload: {} }),
    ).toMatchObject({
      from: "Unknown sender",
      subject: "(no subject)",
      to: [],
